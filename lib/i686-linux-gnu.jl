...
function Base.getproperty(x::Ptr{__JL_Ctag_97}, f::Symbol)
...
function Base.getproperty(x::__JL_Ctag_97, f::Symbol)
    r = Ref{__JL_Ctag_97}(x)
    ptr = Base.unsafe_convert(Ptr{__JL_Ctag_97}, r)
    fptr = getproperty(ptr, f)
    GC.@preserve r unsafe_load(fptr)
end

function Base.setproperty!(x::Ptr{__JL_Ctag_97}, f::Symbol, v)
...
function Base.getproperty(x::Ptr{__JL_Ctag_98}, f::Symbol)
    f === :max_retries && return Ptr{Csize_t}(x + 0)
    f === :backoff_scale_factor_ms && return Ptr{UInt32}(x + 4)
    f === :max_backoff_secs && return Ptr{UInt32}(x + 8)
    f === :jitter_mode && return Ptr{aws_exponential_backoff_jitter_mode}(x + 12)
    f === :initial_bucket_capacity && return Ptr{Csize_t}(x + 16)
    return getfield(x, f)
end

function Base.getproperty(x::__JL_Ctag_98, f::Symbol)
    r = Ref{__JL_Ctag_98}(x)
    ptr = Base.unsafe_convert(Ptr{__JL_Ctag_98}, r)
    fptr = getproperty(ptr, f)
    GC.@preserve r unsafe_load(fptr)
end

function Base.setproperty!(x::Ptr{__JL_Ctag_98}, f::Symbol, v)
    unsafe_store!(getproperty(x, f), v)
end
...
function Base.getproperty(x::Ptr{aws_s3_client_config}, f::Symbol)
    ...
    f === :region && return Ptr{aws_byte_cursor}(x + 4)
    f === :client_bootstrap && return Ptr{Ptr{aws_client_bootstrap}}(x + 12)
    f === :tls_mode && return Ptr{aws_s3_meta_request_tls_mode}(x + 16)
    f === :tls_connection_options && return Ptr{Ptr{aws_tls_connection_options}}(x + 20)
    f === :fio_opts && return Ptr{Ptr{aws_s3_file_io_options}}(x + 24)
    f === :signing_config && return Ptr{Ptr{aws_signing_config_aws}}(x + 28)
    f === :part_size && return Ptr{UInt64}(x + 32)
    f === :max_part_size && return Ptr{UInt64}(x + 40)
    f === :multipart_upload_threshold && return Ptr{UInt64}(x + 48)
    f === :throughput_target_gbps && return Ptr{Cdouble}(x + 56)
    f === :memory_limit_in_bytes && return Ptr{UInt64}(x + 64)
    f === :retry_strategy && return Ptr{Ptr{aws_retry_strategy}}(x + 72)
    f === :retry_config && return Ptr{__JL_Ctag_98}(x + 76)
    f === :compute_content_md5 && return Ptr{aws_s3_meta_request_compute_content_md5}(x + 96)
    f === :shutdown_callback && return Ptr{Ptr{aws_s3_client_shutdown_complete_callback_fn}}(x + 100)
    f === :shutdown_callback_user_data && return Ptr{Ptr{Cvoid}}(x + 104)
    f === :proxy_options && return Ptr{Ptr{aws_http_proxy_options}}(x + 108)
    f === :proxy_ev_settings && return Ptr{Ptr{proxy_env_var_settings}}(x + 112)
    f === :connect_timeout_ms && return Ptr{UInt32}(x + 116)
    f === :tcp_keep_alive_options && return Ptr{Ptr{aws_s3_tcp_keep_alive_options}}(x + 120)
    f === :monitoring_options && return Ptr{Ptr{aws_http_connection_monitoring_options}}(x + 124)
    f === :enable_read_backpressure && return Ptr{Bool}(x + 128)
    f === :initial_read_window && return Ptr{Csize_t}(x + 132)
    f === :enable_s3express && return Ptr{Bool}(x + 136)
    f === :s3express_provider_override_factory && return Ptr{Ptr{aws_s3express_provider_factory_fn}}(x + 140)
    f === :factory_user_data && return Ptr{Ptr{Cvoid}}(x + 144)
    f === :network_interface_names_array && return Ptr{Ptr{aws_byte_cursor}}(x + 148)
